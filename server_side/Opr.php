...
class Opr {
	 ...
	 * @param string  $name      the user's defined name
	 * @return query result
	 */
	public function add_item_watch_list( $user_id, $item_id, $info_id, $name ) {
		$query ="INSERT INTO ".self::ITEM_WATCHLIST_TB." VALUES ('$user_id','$item_id','$info_id','0','$name')";
		return self::$db -> query_and_check( $query );
	}
	/**
	 ...
	 * @return query result
	 */
	public function check_item_watch_list( $user_id ) {
		$query ="SELECT fk_item_id, fk_info_id, name, available FROM ".self::ITEM_WATCHLIST_TB." WHERE fk_user_id='$user_id'";
		$result= self::$db -> query( $query );
		if ( mysqli_num_rows( $result )>0 ) {
			$data=array();
			while ( $row = $result -> fetch_assoc() ) {
	 ...
	 * @return query result
	 */
	public function remove_item_watch_list( $user_id, $item_id, $info_id ) {
		$query ="DELETE FROM ".self::ITEM_WATCHLIST_TB." WHERE fk_user_id='$user_id' AND fk_item_id='$item_id' AND fk_info_id='$info_id'";
...
" );
	 ...
	 * @return query result
	 */
	public function add_promotion_watch_list( $user_id, $item_id, $info_id, $name ) {
		$query ="INSERT INTO ".self::PROMOTION_WATCHLIST_TB." VALUES ('$user_id','$item_id','$info_id','0','$name')";
		return self::$db -> query_and_check( $query );
	}
	/**
	 ...
	 * @return query result
	 */
	public function check_promotion_watch_list( $user_id ) {
		$query ="SELECT  fk_item_id, fk_info_id, name, available  FROM ".self::PROMOTION_WATCHLIST_TB." WHERE fk_user_id='$user_id'";
		$result= self::$db -> query( $query );
		if ( mysqli_num_rows( $result )>0 ) {
			$data=array();
			while ( $row = $result -> fetch_assoc() ) {
	 ...
	 */
	public function remove_promotion_watch_list( $user_id, $item_id, $info_id ) {
		$query ="DELETE FROM ".self::PROMOTION_WATCHLIST_TB." WHERE fk_user_id='$user_id' AND fk_item_id='$item_id' AND fk_info_id='$info_id'";
		return self::$db -> query_and_check( $query );
	}
	/**
